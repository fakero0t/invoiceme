...
import { Pool } from 'pg';
import { UpdateLineItemCommand } from './UpdateLineItemCommand';
import { IInvoiceRepository } from '../../../../domain/invoice/IInvoiceRepository';
import { IEventBus } from '../../../../domain/shared/IEventBus';
import { LineItemUpdatedEvent } from '../../../../domain/invoice/events/LineItemUpdatedEvent';
import { NotFoundException } from '../../../../domain/shared/DomainException';

@injectable()
export class UpdateLineItemCommandHandler {
  constructor(
  ...
  async handle(command: UpdateLineItemCommand): Promise<void> {
    ...
    try {
      ...
      // Update line item (domain method)
      invoice.updateLineItem(command.lineItemId, {
      ...
      // Publish event
      await this.eventBus.publish(new LineItemUpdatedEvent(invoice.id, command.lineItemId));
